...
class GerenciadorDeDados:
    ...
    def obter_todos_dados(self):
        with self.obter_cursor() as cursor:
            cursor.execute('SELECT id, valor, data_criacao FROM dados ORDER BY data_criacao DESC')
    ...
    def obter_usuario_por_nome(self, nome_usuario):
        with self.obter_cursor() as cursor:
            cursor.execute('SELECT id, nome_usuario, hash_senha FROM usuarios WHERE nome_usuario = ?', (nome_usuario,))
